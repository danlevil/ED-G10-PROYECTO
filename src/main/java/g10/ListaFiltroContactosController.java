...
import FIltradores.FiltradorPersonas;
 ...
 * @author Charlie
 */
public class ListaFiltroContactosController implements Initializable {
    ...
    private FiltradorPersonas f1;
    private List<Contacto> soloPersonas;
    /**
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
        f1= new FiltradorPersonas();

        soloPersonas= f1.filtrarSoloPersonas(Agenda.contactosMaster);
...
private void mostrarElementos() {
        ...
        //int fin = Math.min(inicio + ELEMENTOS_POR_PAGINA, solofavs.size());
        int fin = Math.min(inicio + ELEMENTOS_POR_PAGINA, soloPersonas.size());
        for (int i = inicio; i < fin; i++) {
            ...
            agregarHBox(soloPersonas.get(i));
    ...
    private void avanzarIzquierda(MouseEvent event) {
        ...
        int totalPaginas = (int) Math.ceil((double) soloPersonas.size() / ELEMENTOS_POR_PAGINA);
